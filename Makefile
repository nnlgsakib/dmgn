...
build-darwin:
	...
	CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w" -o dmgn-darwin-arm64 ./cmd/dmgn
...
release:
	...
	@CGO_ENABLED=0 GOOS=darwin GOARCH=arm64 go build -ldflags="-s -w -X main.version=$(git describe --tags)" -o release/dmgn-darwin-arm64 ./cmd/dmgn
